...
CODIGO FUNCINANDO PERFEITAMENTE
...
const App = () => {
  const [iconeImage, setIconeImage] = useState('icone.png'); // Nome da imagem do ícone padrão

  // Função para atualizar a imagem do ícone com base nos dados do backend
  const atualizarIcone = novaImagem => {
    setIconeImage(novaImagem);
  };
    ...
    <View style={styles.container}>
      {
        <View style={styles.container}>
          <Image
            source={require('./images/Halu.jpg')}
          ...
          <View style={styles.checkboxContainer}>
            {diasDaSemana.map((ativo, index) => (
              <View key={index} style={diaStyle(ativo)}>
                <Text style={styles.diaTexto}>{diasAbreviados[index]}</Text>
              </View>
            ))}
          </View>
          <Text style={styles.categoriasEntrega}>
      ...
      <View style={styles.iconeContainer}>
        <Image
          source={require(`./background/pizzabackground.png`)}
          style={styles.icone}
        />
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  iconeContainer: {
    position: 'absolute',
    top: 300, // Ajuste o posicionamento vertical conforme necessário
    left: -150, // Ajuste o posicionamento horizontal conforme necessário
  },
  icone: {
    width: 300, // Defina o tamanho do ícone conforme necessário
    height: 300,
  },
});
